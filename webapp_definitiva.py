...
if st.session_state.stato_quiz == 'setup':
    st.title("📂 Configurazione Test per CUC")
    ...
    if scelta_file == "Carica file .txt":
        ...
        # Trova tutti i file che iniziano con DATABASE e finiscono con .txt
        ...
        for f_name in sorted(files):
            try:
                ...
                pass
        if not domande_totali:
            st.error("Nessun file DATABASE trovato nella cartella.")
        ...
        if os.path.exists(scelta_file):
            ...
            st.error(f"File '{scelta_file}' non trovato.")

    # Se abbiamo caricato domande, mostriamo le opzioni di filtro
    if domande_totali:
        ...
        # Disabilita/Nasconde range se selezioni "Tutti i DATABASE"
        if not usa_tutti:
            ...
            st.info("ℹ️ Modalità 'Tutti i DATABASE': l'intervallo manuale è disattivato per includere l'intero archivio.")
...
# ===============================
# 4. Quiz in Corso (Logica invariata)
# ===============================
    ...
    with st.sidebar:
        ...
        if st.button("Termina"): st.session_state.stato_quiz = 'fine'; st.rerun()
